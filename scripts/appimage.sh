...
cp "demo/${TARGET}" "$APPDIR/bin/"
cp -r assets levels deps "$APPDIR/"
rm -rf "$APPDIR/deps/include"
...
cd \$HERE
export LD_LIBRARY_PATH="\$HERE/deps/lib:\$LD_LIBRARY_PATH"
...
if command -v magick &> /dev/null; then
  ...
  echo "⚠️  Warning: 'magick' not found. Creating empty icon."
  touch "$ICON_PATH"
fi
