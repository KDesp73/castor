    ...
    for (int x = 0; x < ctx->screen_width; x++) {
        ...
        while (!hitWall && distance < 20.0) {
            ...
            if (tile > 0) {
                ...
                if (fabs(rayX) > fabs(rayY)) {
                    textureOffset = fmod(hitY, 1.0);  // Vertical wall
                } else {
                    textureOffset = fmod(hitX, 1.0);  // Horizontal wall
                }
            }
        ...
        float correctedDistance = distance * cos((x - ctx->screen_width / 2) * (ctx->fov / ctx->screen_width) * M_PI / 180.0);
        ...
        if (ctx->textures[tile] != NULL) {
            ...
            SDL_Rect dstRect = { x, wallTop, 1, wallBottom - wallTop > 0 ? wallBottom - wallTop : 1 };
